...
export default function TabAi({ template, nextStepClick, saveAndExit, data, fireEvent }: TabContentProps) {
  ...
  const debouncedPrompt = useDebounce(data?.description ?? '', 900);
  ...
  const deleteFile = (id: number) => {
    const photos = data?.photos ? data.photos.filter((preview: any) => preview.lastModified !== id) : [];
  ...
  const matches: tMatch[] = [
    {
      ...
      transformChild: child => (
        <Input
          ...
          onChange={e => fireEvent({ description: e.currentTarget.value })}
        />
      ),
    },
    {
