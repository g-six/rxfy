...
export default function RxTable({
  ...
  const TableHeadComponents = elements.filter(
    (el) => el.props.className !== rowClassName
  );

  const row_items = elements.filter(
    (el) => el.props.className === rowClassName
  );
    ...
    <div className='building-and-sold-column'>
      ...
      {data.map((item) => {
        return (
          ...
          >
            {row_items[0].props.children.map(
              (unit: ReactElement) => {
                if (unit.props.children === '{Other Unit No}') {
                  return (
                    <a
                      href={`/property?mls=${item.MLS_ID}`}
                      key={unit.key}
                ...
                if (unit.props.children === '{Other Sqft}') {
                  return (
                    <span
                      ...
                      className={unit.props.className}
                    >
                      {formatValues(item, 'L_FloorArea_Total')}
                    </span>
                  );
                }
                if (unit.props.children === '{Other Sqft}') {
