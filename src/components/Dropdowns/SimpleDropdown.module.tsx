    ...
    if (['nav', 'div'].includes(c.type as string) && typeof c.props?.children !== 'string') {
      ...
      if (props.className?.split(' ').includes('w-dropdown-list')) {
        return (
          ...
          >
            <Menu.Items className='absolute right-0 top-12 z-10 mt-2 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
              <div className='py-1'>
