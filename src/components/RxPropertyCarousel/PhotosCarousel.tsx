...
export default function PhotosCarousel({ propertyPhotos }: Props) {
  ...
  return (
    <div
      className={`${styles.PhotoCarousel} PhotoCarousel fixed z-[1001] top-0 left-0 w-screen h-screen  ${
        show ? 'bg-black/90 opacity-100' : 'bg-black/0 opacity-0 pointer-events-none'
      } p-5 gap-5`}
    >
      <div className='flex justify-end'>
        <button onClick={closeModal} className='bg-transparent  z-[1001]'>
      ...
      <div className='w-full h-full  mx-auto relative overflow-hidden'>
        <button
          id='slider-next'
          className='absolute right-0 cursor-pointer z-[1001] h-full px-2 opacity-70 transition-all duration-300  bg-black/0 hover:opacity-100 hover:bg-black/10'
        >
          <ChevronRightIcon className='w-14 h-14 text-white ' />
        </button>
        <button
          id='slider-prev'
          className={`absolute left-0 cursor-pointer z-[1001] h-full px-2 opacity-70 transition-all duration-300  bg-black/0 hover:opacity-100 hover:bg-black/10`}
        >
      ...
      <div className='h-full flex-shrink-0 w-full flex justify-center mx-auto overflow-hidden z-[1001]'>
        <Swiper
          onSwiper={(swiper: SwiperType) => {
