...
export default function MortgageCalculator({
  ...
  const [state, setState] = React.useState({
    ...
    formattedPrice: property.AskingPrice
      ? formatCurrency(property.AskingPrice)
      : 0,
    formattedDownpayment: property.AskingPrice
      ? formatCurrency(property.AskingPrice * 0.05)
      : 0,
    downpayment: property.AskingPrice
      ? property.AskingPrice * 0.05
      : 0,
    downpaymentPct: 5,
    rate: 8,
    years: 30,
  ...
  const inputFocused = (e: React.FocusEvent<HTMLInputElement>) => {
    if (
      ['downpayment', 'downpaymentPct'].indexOf(
    ...
    if (isNumeric(e.currentTarget.value)) {
      if (key === 'downpayment') {
        const downpaymentPct =
          e.currentTarget.value && state.price
            ? roundUpFinance(
                (parseFloat(e.currentTarget.value) / state.price) *
                  100
              )
            : 0;
        setState({
          ...state,
          downpaymentPct,
          formattedDownpayment: formatCurrency(
        ...
        const dp =
          state.price * (parseFloat(e.currentTarget.value) / 100);
        setState({
          ...state,
          downpaymentPct: roundUpFinance(
            parseFloat(e.currentTarget.value)
          ),
